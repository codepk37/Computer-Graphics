...
std::vector<Surface> createSurfaces(std::string pathToObj, bool isLight, uint32_t shapeIdx)
{
    ...
    for (size_t s = 0; s < shapes.size(); s++) {
        ...
        // std::cout<<" material size "<<materialIds.size()<<std::endl;
        ...
        if (materialIds.size() == 0) {
            ...
            if (matId != -1) {
                ...
                surf.diffuse = Vector3d(1, 1, 1);
            }
        }

        // std::cout<<" surf.difftextire.data  "<<surf.diffuseTexture.data<<std::endl;
...
Interaction Surface::rayTriangleIntersect(Ray ray, Vector3d v1, Vector3d v2, Vector3d v3, Vector3d n)
{
    ...
    if (si.didIntersect) {
        ...
        if (edge1 && edge2 && edge3) {
            ...
            si.v1=v1;
            si.v2=v2;//store tri vertices in interaction
            si.v3=v3;
            si.alpha= Cross( v3-si.p , v2-si.p  ).Length() / Cross(v3-v1,v2-v1).Length();
            si.beta = Cross( v3-si.p  ,v1-si.p ).Length() / Cross(v3-v1,v2-v1).Length();
            si.gamma =Cross( v1-si.p  ,v2-si.p ).Length() / Cross(v3-v1,v2-v1).Length();
...
void Surface::intersectBVH(uint32_t nodeIdx, Ray& ray, Interaction& si) //on triangle
{
    BVHNode& node = this->nodes[nodeIdx];
    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            ...
            siIntermediate.uv1=this->tris[this->getIdx(i + node.firstPrim)].uv1;
            siIntermediate.uv2=this->tris[this->getIdx(i + node.firstPrim)].uv2;
            siIntermediate.uv3=this->tris[this->getIdx(i + node.firstPrim)].uv3; //also adding uv info in interaction
            
            siIntermediate.hastexture=this->diffuseTexture.data;
            // std::cout<<" has texture "<<siIntermediate.hastexture<<std::endl;

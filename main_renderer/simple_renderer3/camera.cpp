...
Ray Camera::generateRay(int x, int y) {
    float rand1=next_float(); //random sampling x and y, instead of midpoint
    float rand2=next_float(); //DONE
    Vector3f pixelCenter = this->upperLeft +  (  rand1*this->pixelDeltaU + rand2*this->pixelDeltaV);//0.5f * (this->pixelDeltaU + this->pixelDeltaV);
